...
class NavExplorerAgent(SolverAgent):
    ...
    def goal_str(self) -> str:
        return dedent("""\
            ...
            Every navigation task MUST follow this format: "Navigate to <specific page name> on <website>"
            - Be specific about the destination page (e.g., "Navigate to the 2023 MacBook Pro product page on Amazon" not just "Navigate to a product page")
            - Include enough context so the agent knows exactly which page to find

            Examples of GOOD navigation tasks:
            - "Navigate to the 2023 MacBook Pro 14-inch product page on Amazon"
            - "Navigate to my shopping cart on eBay"
            - "Go to the Customer Service contact form on United Airlines"
            - "Navigate to the laptop section with filters for Dell, under $1000 on Best Buy"

            Examples of BAD navigation tasks:
            - "Navigate to a page" -- which page?
            - "Go to home" -- too trivial
            - "Click a link" -- not a task, just an action
            ...
            ## Verification
            Before adding a navigation task, try clicking the link/button to verify it actually leads to the expected page. After verifying, navigate back using `go_back()` and then add the task.
